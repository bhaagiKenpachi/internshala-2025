    ...
    return (
        ...
        >
            <div className="max-w-6xl w-full">
                <h2 className="font-bold mb-12 text-4xl md:text-5xl xl:text-6xl text-[#1e4145] pb-4 flex gap-2 items-center">How I Help</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {services.map((service) => (
                        <div
                            key={service.title}
                            className="bg-[#bfe1e7] border border-gray-400 rounded-xl shadow-md flex flex-col items-stretch p-8 transition-transform hover:scale-105"
                        >
                            <img
                                src={service.image}
                                alt={service.title}
                            ...
                            />
                            <h3 className="font-bold mb-2 mt-2 text-2xl flex justify-between items-center text-[#1e4145]">{service.title}</h3>
                            <p className="font-normal text-[#1e4145] !text-base mb-6 flex-1">{service.description}</p>
                            <button className="border border-[#1e4145] text-[#1e4145] py-2 rounded-md hover:bg-white transition-colors">Learn More</button>
                        </div>
                    ))}
                </div>
            </div>
